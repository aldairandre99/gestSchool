<x-app-layout>
    <x-page-header :title="__('Final results')">
        <x-slot name="subtitleSlot">
            <x-turma-label :turma="$turma" :showAno="true" />
        </x-slot>
        <x-slot name="actions">
            <x-btn variant="danger" icon="file-down" :href="route('pautas.situacao.pdf', array_merge(['turma' => $turma], ['peso_t1' => $calc->pesos[0], 'peso_t2' => $calc->pesos[1], 'peso_t3' => $calc->pesos[2]]))">{{ __('Export PDF') }}</x-btn>
